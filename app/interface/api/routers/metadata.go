...
import (
	...
	"github.com/PercyPenguin-Metadata/app/contracts"
	"github.com/PercyPenguin-Metadata/app/domain/metadata"
...
func HandleMetadataRequest(ethClient *ethereum.EthereumClient, address string, configService *config.ConfigService) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		instance, err := contracts.NewPercyPenguin(common.HexToAddress(address), ethClient.Client)
		if err != nil {
			render.Status(r, 500)
			render.JSON(w, r, err)
			log.Errorln(err)
		...
		genomeInt, err := instane.GeneOf(nil, big.NewInt(int64(iTokenId)))
		if err != nil {
			render.Status(r, 500)
			render.JSON(w, r, err)
			log.Errorln(err)
